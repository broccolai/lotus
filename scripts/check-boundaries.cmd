...
if %ERRORLEVEL% EQU 0 (
    echo Raw Windows or unsafe code is only allowed in crates\lotus-windows and isolated shell bridges. 1>&2
    exit /b 1
)
...
if %ERRORLEVEL% EQU 0 (
    echo The windows dependency is only allowed in crates\lotus-windows and isolated shell bridges. 1>&2
    exit /b 1
)
if %ERRORLEVEL% GTR 1 exit /b %ERRORLEVEL%
exit /b 0
