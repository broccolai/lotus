use super::{
    AppError, DockBadge, DockContextRequest, DockHitTarget, DockIcon, DockMetrics, DockScene,
    DockSettings, NativeIconCache, NotificationBadgeStyle, NotificationSource, Path, SettingsStore,
    SignedPoint, SvgAsset, WindowHandle, WindowInfo, adapt_dock_items_with_native,
    execute_activation, foreground_window, resolve_executable, show_error,
};
...
impl DockRuntime {
    ...
    pub(super) fn rebuild(&mut self, windows: &[WindowInfo]) {
        self.model.rebuild(projected_items(self.model.settings(), windows));
        self.refresh_scene_items();
    }

    pub(super) fn set_dpi(&mut self, dpi: u32) -> Result<(), AppError> {
        self.scene.set_dpi(dpi).then_some(()).ok_or(AppError::InvalidScene)?;
        self.refresh_scene_items();
        Ok(())
    }
    ...
    pub(super) fn refresh_scene_items(&mut self) {
        let icon_size = self.scene.icon_size_pixels().saturating_mul(NATIVE_ICON_SAMPLE_SCALE);
        let mut scene_items = adapt_dock_items_with_native(self.model.items(), |_, item| {
            self.native_icons.icon(Path::new(&item.icon_source), icon_size).ok().flatten()
        });
        for item in &mut scene_items {
            let Some(source) = self.model.items().get(item.source_index()) else { continue };
            ...
            let badge = match (self.model.settings().notification_badge_style, count) {
                ...
                (NotificationBadgeStyle::Count, count)
                    if notification_count.is_lower_bound && count == notification_count.value =>
                {
    ...
    pub(super) fn apply_settings(
        ...
        if impact.changed {
            let mut scene = DockScene::new(dpi, metrics, mascot(self.model.settings()), Vec::new())
                .ok_or(AppError::InvalidScene)?;
    ...
    pub(super) fn hit_test(&self, x: i32, y: i32) -> Option<DockHitTarget> {
        ...
        self.scene.layout(size.width(), size.height()).hit_test(x, y)
    }

    pub(super) fn jirachi_menu_anchor(&self, request: DockContextRequest) -> Option<SignedPoint> {
        let DockContextRequest::Pointer { screen, client } = request else { return None };
        ...
        let center_x = i32::try_from(jirachi.left.saturating_add(jirachi.width / 2)).ok()?;
        ...
        Some(SignedPoint {
            ...
            y: screen.y.saturating_sub(client.y).saturating_add(top).saturating_add(overlap),
        })
    }

    pub(super) fn pointer_moved(&mut self, x: i32, y: i32) -> bool {
        let target = self.hit_test(x, y);
        self.interaction.pointer_moved(&mut self.scene, target, x, y)
    }
    ...
    pub(super) fn pointer_pressed(&mut self, x: i32, y: i32) -> bool {
        let target = self.hit_test(x, y);
        self.interaction.pointer_pressed(&mut self.scene, target, x, y)
    }
    ...
    pub(super) fn pointer_released(
        ...
        let mut changed = self.scene.set_pressed(None) | self.scene.set_hovered(released_over);
        self.interaction.release();

        if let Some(drag) = self.scene.drag() {
            changed |= self.scene.update_drag(x, y);
            let size = self.scene.desired_size();
            let insertion_slot = self.scene.drag_insertion_slot(size.width(), size.height());
            let source_index = drag.source_index;
            let layout = self.scene.layout(size.width(), size.height());
            let visible_sources =
                layout.items.iter().map(|item| item.source_index).collect::<Vec<_>>();
        ...
        Ok((changed, (pressed == released_over).then_some(pressed).flatten()))
    }
    ...
    pub(super) fn activate(&self, target: DockHitTarget, owner: WindowHandle) {
        let DockHitTarget::Item(source_index) = target else { return };
        let foreground = foreground_window();
        let Some((decision, item)) = self.model.activation(source_index, foreground.as_ref())
        else {
            return;
        };
...
fn mascot(settings: &DockSettings) -> DockIcon {
    settings
        .mascot_image_path
        .as_deref()
        .and_then(|path| lotus_windows::custom_image::load_custom_image(Path::new(path)).ok())
